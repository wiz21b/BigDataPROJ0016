...
class SEIR_HCD_1(Model):
    ...
    def get_initial_parameters(self, randomPick = False, picks = 1000):
        ...
        # ----------------------------------
        bestParams = [rho_0, sigma_0, tau_0, delta_0, theta_0,theta2_0, gamma1_0, gamma2_0,
        ...
        theta2_bounds = [theta2_min, bestParams['Theta2'], theta2_max]
        mu_bounds = [mu_min, bestParams['Mu'], mu_max]
        eta_bounds = [eta_min, bestParams['Eta'], eta_max]

        bounds = [rho_bounds, sigma_bounds, tau_bounds, delta_bounds,
    ...
    def plumb(self, parameters):
        print("Optimizing: {}".format(self._optim))
        self._optim += 1
        sres = np.array([])
        periods = [(12, 63), (64, 98), (99, 145), (146, 206), (207, 218), (219, 245), (246, 274), (275, 331), (332, 364), (365, 390), (391, 404)] # we start fitting from the 2nd period to start with higher values
        # solution 2, here start from 0. but use the 0 to compute the date so not cool... et marche moins bien que sol 1
    
        ms = SEIR_HCD_2(stocha = False, cstParams=parameters)
        ms.set_IC(conditions = self._IC)
        for period in periods:
            ms.fit_parameters(data= self._data[period[0]:period[1], :], randomPick = False, picks = 1000)
        ...
        residuals = np.abs(residuals_error(sres[0:395,StateEnum.HOSPITALIZED.value],self._data[0:395,ObsEnum.NUM_HOSPITALIZED.value]))
        residuals = np.sum(residuals)
        residuals2 = np.abs(residuals_error(sres[0:395,StateEnum.CRITICAL.value],self._data[0:395,ObsEnum.NUM_CRITICAL.value]))
        residuals2 = np.sum(residuals2)
        residuals3 = np.abs(residuals_error(sres[0:395,StateEnum.FATALITIES.value],self._data[0:395,ObsEnum.NUM_FATALITIES.value]))
        residuals3 = np.sum(residuals3)
        ...
        if res <= self._best:
            ...
            print("Best so for:")
            print("Parameters: ")
            print(dict(zip(self._paramNames, parameters)))
            print("Score: {}".format(res))
            print()
        return res

class SEIR_HCD_2(Model):
    """ 'stocha' -> modèle stochastique ou pas
        'immunity' -> Les gens développent une immunité ou pas
        'errorFct' à fournir si pas stochastique
        'nbExpériments' pour le fit
    """
    def __init__ (self, stocha = False, immunity = True, errorFct = None, nbExperiments = 100,
                  cstParams = None):
        super().__init__(stocha = stocha, errorFct = errorFct, nbExperiments = nbExperiments)
        self._immunity = immunity # ne sert pas à l'instant xar posait un problème
                                  # si set à False car alors on avait un paramètre
        ...
        self._cst = dict(zip(self._paramCstName, cstParams))
    ...
    def model(self, state, parameters):
        ...
        N = self._population
        gamma1 = self._cst['Gamma1']
        gamma2 = self._cst['Gamma2']
        gamma3 = self._cst['Gamma3']
        gamma4 = self._cst['Gamma4']
        beta = parameters['Beta']
        tau = self._cst['Tau']
        delta = self._cst['Delta']
        sigma = self._cst['Sigma']
        rho = self._cst['Rho']
        theta = self._cst['Theta']
        theta2 = self._cst['Theta2']
        mu = self._cst['Mu']
        eta = self._cst['Eta']
        alpha = 0
        if not(self._immunity):
        ...
        if self._stochastic:
            ...
            thetaH = self.population_leave(theta2, H)
